...
days <- c("Sunday", "Monday","Tuesday", "Wedsnesday", "Thursday", "Friday", "Saturday")
...
for (i in 1:7){
  if(week1_sleep[i]>8){
    goodsleep<- 1+goodsleep
  }
}

notes = paste(as.character(goodsleep),(" days had over 8 hours of sleep"))
print(notes)
...
weighted_grades <- grades * 0.1
weighted_grades
grades<-weighted_grades+grades
grades
